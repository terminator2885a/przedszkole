<?php
    ...
    $query = "SELECT id_artykulu, tytul_artykulu, tresc_artykulu, CONCAT(imie, ' ', nazwisko) AS autor_artykulu FROM artykuly JOIN nauczyciele ON autor_artykulu = id_nauczyciela ORDER BY id_artykulu DESC";
...
<head>
    ...
    <title>Wydarzenia - Niepubliczne Przedszkole "Małe Skrzaty" w Łodzi</title>
</head>
<body>
    <div class="page">
        <header>
        ...
        <nav>
            <div class="nav__links">
                ...
                <div class="nav__link current"><a href="wydarzenia.php">Wydarzenia</a></div>
        ...
        <main>
            <article>
                <!-- Miejsce na  kod php-->
                 <h2>Wydarzenia</h2>
                 <p>Zachęcamy do przeczytania, co wydarzyło się w naszym przedszkolu</p>
                <div class="events">
                <?php
                    $result = $conn->query($query);
                    while ($row = $result->fetch_assoc()){
                        echo '<div class="event">';
                        echo '<h4><a href="wydarzenie.php?id_wydarzenia='. $row['id_artykulu'] .'">' . $row['tytul_artykulu'] . '</a></h4>';
                        echo '<span class="author"><em>'. $row['autor_artykulu']. '</em></span>';
                        echo '<p>'. substr($row['tresc_artykulu'], strpos($row['tresc_artykulu'], '<p>') + 3, strpos($row['tresc_artykulu'], '</p>') - strpos($row['tresc_artykulu'], '<p>') - 3) . '.. <a href="wydarzenie.php?id_wydarzenia='. $row['id_artykulu'] .'">Czytaj dalej <i class="fa-solid fa-arrow-right"></i></a></p>';
                        echo '</div>';
                    }
                 ?>
                </div>
