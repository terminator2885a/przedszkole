<?php
    session_start();
    if(isset($_GET['id_wydarzenia'])) $event_id = $_GET['id_wydarzenia'];
    else {header('Location: wydarzenia.php'); exit();}

    require_once 'db/connect.php';
    $conn = new mysqli($host, $user, $pass, $db);

    $query = "SELECT id_artykulu, tytul_artykulu, tresc_artykulu, CONCAT(imie, ' ', nazwisko) AS autor_artykulu, id_rangi, nazwa_rangi FROM artykuly JOIN nauczyciele ON autor_artykulu = id_nauczyciela JOIN rangi ON ranga = id_rangi WHERE id_artykulu=" . $event_id;
...
<body>
    <div class="page">
        ...
        <main>
            <aside>
                <a href="wydarzenia.php" class="aside__link"><i class="fa-solid fa-arrow-left"></i>Wróć do listy artykułów</a>
            </aside>
            <article>
                <?php
                    echo $row['tresc_artykulu'];
                    echo '<p><em>Dodano przez: ' . $row['autor_artykulu'] . ' (' . $row['nazwa_rangi']. ')</em></p>';
                ?>
            </article>
        </main>
        <footer>
