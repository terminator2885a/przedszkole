<?php
    ...
    $query = "SELECT id_grupy, nazwa_grupy, opis_grupy, CONCAT(n1.imie, ' ', n1.nazwisko) AS 'wych1', CONCAT(n2.imie, ' ', n2.nazwisko) AS 'wych2' FROM grupy LEFT JOIN nauczyciele n1 ON grupy.wychowawca1 = n1.id_nauczyciela LEFT JOIN nauczyciele n2 ON grupy.wychowawca2=n2.id_nauczyciela WHERE id_grupy=" . $group;
...
<head>
    ...
    <title>Grupa <?php echo $row['nazwa_grupy']; ?> - Niepubliczne Przedszkole "Małe Skrzaty" w Łodzi</title>
</head>
<body>
    <div class="page">
        <header>
        ...
        <main>
            <aside>
                <!-- Aside generowany przez php -->
                 <div class="aside__link"><a href="grupy.php">Grupy</a></div>
                 <?php
                    for($i=0; $i<count($_SESSION['records']); $i++){
                        echo '<div class="aside__link';
                        if($i == $group-1) echo ' current';
                        echo '"><a href="grupa.php?grupa=' . $_SESSION['records'][$i]['id_grupy'] . '">' . $_SESSION['records'][$i]['nazwa_grupy'] . '</a></div>';
                    }
                 ?>
            </aside>
            <article>
                <?php
                    ...
                    echo '<h2>' . $row['nazwa_grupy'] .'</h2>';
                    echo '<p>'. $row['opis_grupy'] . '</p>';
                    echo '<h4>Wychowawcy: </h4>';
                    echo '<div class="figures">';
                    echo '<figure class="figure__teacher">';
                    echo '<div class="teacher-banner__img">';
                    echo '<img src="img/teachers/' . removePolishCharacters(strtolower(str_replace(' ', '_', $row['wych1'])) . '.jpg">');
                    echo '</div>';
                    echo '<figcaption>'. $row['wych1'];
                    echo '</figure>';
                    if($row['wych2']!=''){
                        echo '<figure class="figure__teacher">';
                        echo '<div class="teacher-banner__img">';
                        echo '<img src="img/teachers/' . removePolishCharacters(strtolower(str_replace(' ', '_', $row['wych2'])) . '.jpg">');
                        echo '</div>';
                        echo '<figcaption>'. $row['wych2'];
                        echo '</figure>';
                    }
                    echo '</div>';
                ?>
