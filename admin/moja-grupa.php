...
<head>
    ...
    <title>Moja grupa - Niepubliczne Przedszkole "Małe Skrzaty" w Łodzi</title>
</head>
<body>
    <div class="page">
        <header>
        ...
        <main>
            <h2>Moja grupa</h2>
            <?php
                echo '<h3>'. $_SESSION['group_name'] .'</h3>';
                echo '<p><q>' . $conn->query('SELECT opis_grupy FROM grupy WHERE nazwa_grupy="'. $_SESSION['group_name'] .'"')->fetch_assoc()['opis_grupy'] . '</q></p>';
                echo '<h3>Lista przedszkolaków</h3>';

                $query = sprintf("SELECT * FROM przedszkolaki JOIN grupy ON przedszkolaki.grupa=grupy.id_grupy WHERE nazwa_grupy='%s' ORDER BY nazwisko", $_SESSION['group_name']);
                $result = $conn->query($query);
