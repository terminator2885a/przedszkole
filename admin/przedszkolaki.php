...
<head>
    ...
    <?php } else{ ?>
    <title>Moje przedszkolaki - Wychowawca w Niepublicznym Przedszkolu "Małe Skrzaty" w Łodzi</title>
    <?php } ?></head>
<body>
    <div class="page">
        <header>
        ...
        <main>
            ...
            <form action="przedszkolaki.php" method="post" class="search_form">
                ...
                <select name="group" id="group">
                    <option value="" disabled selected hidden>Nazwa grupy</option>
                    <?php
                        $query_groups = "SELECT * FROM grupy";
                        $result_groups = $conn->query($query_groups);

                        while($r = $result_groups->fetch_assoc()){
                            echo '<option value="'. $r['id_grupy'] . '">' . $r['nazwa_grupy'] . '</option>';
                        }
                    ?>
                </select>
                <?php } ?>
            ...
            <?php
                if(isset($_POST['name']) || isset($_POST['group'])){
                    ...
                    if(isset($_POST['group'])) {if($_POST['group'] == '') $group_selected = false;
                    else $group_selected = true;}
                }
            ?>
            ...
            <div class="data_show_hide">
                ...
                <?php } else{ ?>
                    <a href="?all=true">Pokaż więcej danych</a>
                <?php } ?>
            </div>
            
            <table id="preschoolers__table">
                <tr>
                    <th>Nr</th>
                    <th>Nazwisko</th>
                    <th>Imię</th>
