...
<body>
    <div class="page">
        ...
        <main>
            ...
            <article>
                <h2>Komunikaty dyrekcji</h2>
                
                <?php
                    $query = "SELECT * FROM komunikaty ORDER BY data_komunikatu, id_komunikatu DESC";
                    $result = $conn->query($query);

                    while($row = $result->fetch_assoc()){
                        echo '<div class="announcement">';
                            echo '<h3>Komunikat z dnia ' . $row['data_komunikatu'] . '</h3>';
                            echo '<p>' . $row['tresc_komunikatu'] . '</p>';
                        echo '</div>';
                    }
                    $result->free_result();
                ?>
